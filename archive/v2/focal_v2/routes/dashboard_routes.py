...
def api_workload():
    ...
    # Batch-fetch Work Type from linked Master WBS Tasks.
    # Work Sessions don't carry Work Type directly — it lives on the task.
    task_ids: set[str] = set()
    for s in raw_sessions:
        task_rel = s["properties"].get("Task", {}).get("relation", [])
        if task_rel:
            task_ids.add(task_rel[0]["id"])

    task_work_types: dict[str, str] = {}
    for tid in task_ids:
        try:
            r = client.get_page(tid)
            if r.ok:
                wt = extract(r.json().get("properties", {}).get("Work Type", {}))
                if wt:
                    task_work_types[tid] = wt
        except Exception:
            pass
    ...
    for s in raw_sessions:
        ...
        # Work Type: try the session itself first, then fall back to the
        # linked Master WBS Task (where Work Type is actually stored).
        task_rel  = props.get("Task", {}).get("relation", [])
        task_id   = task_rel[0]["id"] if task_rel else ""
        work_type = (extract(props.get("Work Type", {}))
                     or task_work_types.get(task_id)
                     or "Unclassified")
