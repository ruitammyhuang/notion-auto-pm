...
def get_valid_names(include_deprecated: bool = False) -> list[str]:
    """Return list of valid work type name strings."""
...
def save_work_type(
    ...
    Raises ValueError if name already exists or color is invalid.
    """
    name = name.strip()
    if not name:
...
def update_work_type(name: str, **fields) -> dict:
    """Update color or description for an existing work type.

    Allowed fields: color, description. Increments version and logs the change.
    """
    store = _reload_if_stale()
    now   = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    for t in store.get("work_types", []):
        if t["name"] == name:
            for k in ("color", "description"):
                if k in fields:
                    if k == "color" and fields[k] not in NOTION_COLORS:
                        raise ValueError(f"color must be one of: {', '.join(NOTION_COLORS)}")
                    t[k] = fields[k]
...
def deprecate_work_type(name: str) -> None:
    """Mark a work type as deprecated (soft-delete)."""
    store = _reload_if_stale()
    now   = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    for t in store.get("work_types", []):
        if t["name"] == name:
